...
router.post(
  ...
  async (req, res) => {
    try {
      console.log('🔍 Create quest request:', {
      ...
      if (error instanceof Error) {
        console.error('❌ Create quest error details:', {
...
router.get(
  ...
  authenticateToken,
  requireUser,
  validateParams(questIdSchema),
  async (req, res) => {
    try {
      const quest = await QuestService.getQuestById(
        req.params.id,
        req.user!.userId
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      console.log('🔍 Complete quest request:', {
      ...
      const monsterResult = await MonsterService.updateMonsterXp(
        req.user!.userId,
        result.xpGained
      );
      ...
      if (error instanceof Error) {
        console.error('❌ Complete quest error details:', {
          message: error.message,
          stack: error.stack,
          questId: req.params.id,
          userId: req.user!.userId,
